...
import { FileImage, Minus, Plus, Search, Eye, RotateCcw, AlertTriangle, ArrowRight, ArrowRightLeft, Upload, X, Receipt, CheckCircle2, XCircle, AlertCircle, Clock, ShieldCheck, FileCheck, QrCode, Camera, Calendar, Package, TrendingUp, Users, ZoomIn, ZoomOut, Zap, Info, RefreshCw } from "lucide-react";
                        ...
                        <DialogContent className="bg-zinc-950 border-zinc-800 text-zinc-100 sm:max-w-2xl max-h-[88vh] overflow-y-auto overflow-x-hidden [&>*]:min-w-0">
                          <DialogHeader className="space-y-2 border-b border-zinc-800 pb-4 text-left">
                            <div className="flex flex-wrap items-center gap-2">
                              ...
                              <Badge className="bg-zinc-800 text-zinc-300 border-zinc-700">1:1 Even Exchange</Badge>
                            </div>
                            <p className="text-xs text-zinc-400">
                              {returnItem.returnDateTime} · Original receipt{" "}
                              <span className="font-semibold text-zinc-200">{returnItem.display_sales_id}</span>
                            </p>
                          </DialogHeader>
                          <div className="space-y-4 pt-1">
                            {/* ITEMS: what went back, what the customer received */}
                            <div className="space-y-3">
                              {returnItem.returnDetails.map((detail) => {
                                const sizeChanged = detail.productSize !== detail.replacementProductSize;
                                  ...
                                  <div key={detail.return_detail_id} className="rounded-xl border border-zinc-800 bg-zinc-900 p-4">
                                    <div className="grid grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-center gap-3">
                                      <div className="min-w-0">
                                        ...
                                        <p className="text-xs text-zinc-400">
                                          {detail.productColor} · Size {detail.productSize} · Qty {detail.quantity_returned}
                                        </p>
                                      </div>
                                      ...
                                      <div className="min-w-0">
                                        ...
                                        <p className="text-xs text-zinc-400">
                                          <span className={colorChanged ? "font-semibold text-yellow-300" : ""}>{detail.replacementProductColor}</span>
                                          {" · "}
                                          <span className={sizeChanged ? "font-semibold text-yellow-300" : ""}>Size {detail.replacementProductSize}</span>
                                    ...
                                    <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1 border-t border-zinc-800 pt-3 text-xs text-zinc-400">
                                      ...
                                      <span>Returned item: <span className="text-zinc-200">{detail.inventory_action}</span></span>
                                    </div>
                                  </div>
                                );
                              })}
                            ...
                            <dl className="grid grid-cols-1 gap-x-6 rounded-xl border border-zinc-800 bg-zinc-900 px-4 py-1 text-sm sm:grid-cols-2">
                              {[
                                ["Customer", returnItem.customerName],
                                ["Sale status", returnItem.salesStatus],
                            ...
                            {(() => {
                              ...
                              return (
                                <div className="flex items-center gap-4 rounded-xl border border-zinc-800 bg-zinc-900 p-3">
                                  {activeReceiptUrl ? (
                                    <a
                                      href={activeReceiptUrl}
                                      target="_blank"
                                      rel="noreferrer"
                                      className="block h-16 w-16 shrink-0 overflow-hidden rounded-lg border border-zinc-700 hover:border-yellow-400/60"
                                    >
                                      <img src={activeReceiptUrl} alt="Receipt proof" className="h-full w-full object-cover" />
                                    </a>
                                  ) : (
                                    <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-lg border border-dashed border-amber-500/40 bg-amber-500/5">
                                      <FileImage className="h-6 w-6 text-amber-400/80" />
                                    </div>
                                  )}
                                  <div className="min-w-0 flex-1">
                                    <p className="text-[10px] font-semibold uppercase tracking-wider text-zinc-400">Receipt proof</p>
                                    {activeReceiptUrl ? (
                                      <>
                                        <p className="truncate text-sm font-medium text-zinc-100">{activeReceiptName}</p>
                                        <p className="text-xs text-zinc-400">Verified {activeVerifiedAt}</p>
                                      </>
                                    ) : (
                                      <p className="text-sm text-amber-300">No receipt photo attached</p>
                                    )}
                                  </div>
                                  <div className="flex shrink-0 items-center gap-2">
                                    {activeReceiptUrl && (
                                      <a
                                        ...
                                        target="_blank"
                                        rel="noreferrer"
                                        className="rounded-md px-2.5 py-1.5 text-xs font-semibold text-yellow-400 hover:bg-zinc-800 hover:text-yellow-300"
                                      >
                                        View
                                      </a>
                                    )}
                                    <label
                                      className={
                                        activeReceiptUrl
                                          ? "inline-flex cursor-pointer items-center gap-1.5 rounded-md border border-zinc-700 px-2.5 py-1.5 text-xs font-medium text-zinc-300 hover:bg-zinc-800 hover:text-white"
                                          : "inline-flex cursor-pointer items-center gap-1.5 rounded-md bg-yellow-400 px-3 py-1.5 text-xs font-bold text-zinc-950 hover:bg-yellow-300"
                                      }
                                    >
                                      <Upload className="h-3.5 w-3.5" />
                                      {activeReceiptUrl ? "Replace" : "Upload"}
                                      {uploadInput}
                                    </label>
                                  </div>
                                </div>
